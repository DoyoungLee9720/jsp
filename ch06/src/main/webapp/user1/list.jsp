...
<body>
	...
	<table border="1">
		...
		<%for(User1VO oneuser : users)
		{%>
		<tr>
			<td><%=oneuser.getUid()%></td>
			<td><%=oneuser.getName()%></td>
			<td><%=oneuser.getBirth()%></td>
			<td><%=oneuser.getHp()%></td>
			<td><%=oneuser.getAge()%></td>
			<td>
				<a href="/ch06/user1/modify.jsp?uid=<%= oneuser.getUid()%>">수정</a>
				<a href="/ch06/user1/delete.jsp?uid=<%= oneuser.getUid()%>" class="del">삭제</a>
			</td>
		</tr>
		<%}%>
	</table>
