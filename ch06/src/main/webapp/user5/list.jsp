...
<body>
	...
	<table border="1">
		...
		<%for(User5VO oneuser : users)
		{%>
		<tr>
			<td><%=oneuser.getSeq()%></td>
			<td><%=oneuser.getName()%></td>
			<td><%=oneuser.getGender()%></td>
			<td><%=oneuser.getAge()%></td>
			<td><%=oneuser.getAddr()%></td>
			<td>
				<a href="/ch06/user5/modify.jsp?seq=<%= oneuser.getSeq()%>">수정</a>
				<a href="/ch06/user5/delete.jsp?seq=<%= oneuser.getSeq()%>" class="del">삭제</a>
			</td>
		</tr>
		<%}%>
	</table>
